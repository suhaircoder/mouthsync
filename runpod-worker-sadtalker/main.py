...
from render_sadtalker import animate_face, backend_info, get_engine

TEMP_DIR = Path(__file__).resolve().parent / "temp_files"
TEMP_DIR.mkdir(parents=True, exist_ok=True)

API_KEY = os.environ.get("WORKER_API_KEY", "").strip()
PRELOAD = os.environ.get("SADTALKER_PRELOAD", "1").strip().lower() in ("1", "true", "yes")
...
async def lifespan(_app: FastAPI):
    if PRELOAD:
        try:
            ...
            # Pod can start; first /infer will surface the error if load still fails
            print(f"[sadtalker] preload failed: {e}", flush=True)
    yield


app = FastAPI(title="MouthSync SadTalker worker", lifespan=lifespan)
