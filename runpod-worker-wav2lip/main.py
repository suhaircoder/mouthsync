...
PRELOAD = os.environ.get("WORKER_PRELOAD", "1").strip().lower() in ("1", "true", "yes")
...
async def lifespan(_app: FastAPI):
    if PRELOAD:
        try:
            ...
            print(f"[wav2lip] preload failed: {e}", flush=True)
    yield


app = FastAPI(title="MouthSync Wav2Lip worker", lifespan=lifespan)
